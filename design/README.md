...
The renderer — `scripts/lib/svg-raster.mjs`, shared with the store artwork below — understands a deliberately small subset of SVG: `<rect>` with a corner radius, `<path>` with M/L/H/V/C/S/Q/T/Z, flat fills and two-stop linear gradients in user space. It throws on anything else rather than dropping it silently, so a shape that needs more than that needs the renderer extended first. The parts the generator addresses by id — `#backdrop` and `#mark` — are named in a comment at the top of the file.

`store/` is the Google Play listing's artwork: `feature-graphic.svg` and the two PNGs beside it, rewritten by `npm run icons` and checked by `npm run icons:check` alongside the app's icons. The feature graphic is drawn at Play's own 1024×500 and rasterised 1:1, which is what lets it lean on whole-numbered edges; because the renderer has no font engine, the wordmark in it is geometry rather than type, and its counters are cut by shapes filled with the background's own gradient. The file's header comment explains both, and [docs/release.md](../docs/release.md) explains where the images go.
